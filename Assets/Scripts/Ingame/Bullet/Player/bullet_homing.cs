...
public class bullet_homing : CBullet
{
    float max_rotate; // 최대 회전각
    GameObject[] targets; // 예비 타겟들
    ...
    GameObject FindClosestEnemy()
    {
        ...
        float predistance = 1000f;
        float distance_in_finding;

        foreach (GameObject select in targets)
        {
            distance_in_finding = Mathf.Sqrt((select.transform.position.x - this.transform.position.x) * (select.transform.position.x - this.transform.position.x)
                                + (select.transform.position.y - this.transform.position.y) * (select.transform.position.y - this.transform.position.y));
            if (distance_in_finding <= predistance)
            {
                closest = select;
            }
            predistance = distance_in_finding;
        }
        return closest;
    }
}

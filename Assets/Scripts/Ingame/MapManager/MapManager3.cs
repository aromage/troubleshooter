...
public class MapManager3 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < r_ground_count; i++)
        {
            ...
            if (r_ground[i] != null)
            {
                r_ground[i].gameObject.transform.position = new Vector3(0f, -640f + 2000f * (i + 13), 0f);
            }
        }
        ...
        for (int i = 0; i < r_ground_count; i++)
        {
            if (r_ground[i] != null)
            {
                r_ground[i].GetComponent<CMap>().re = 3;
    ...
    IEnumerator ControlSpeed()
    {
        ...
        SetMapSpeed(r_ground, 1400);
        yield return new WaitForSeconds(2f);//가속
        SetMapSpeed(map_ground, 400);
        SetMapSpeed(map_sky, 400);
        SetMapSpeed(r_ground, 400);
        yield return new WaitForSeconds(24f); // 보스전 정지
        SetMapSpeed(map_ground, 0);
        SetMapSpeed(map_sky, 0);
        SetMapSpeed(r_ground, 0);
    }
